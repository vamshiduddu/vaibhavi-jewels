        ...
        <form action={saveSettings} className="admin-form">
          <label>
            ...
            <select
              ...
              defaultValue={settings.online_payments_enabled ?? "true"}
            >
          ...
          <p style={{ color: "var(--muted)", fontSize: 12.5, margin: 0 }}>
            Note: online payment also stays off automatically while Razorpay keys are not
            configured on the server, regardless of this setting.
          </p>
          {FIELDS.map((field) => (
            <label key={field.key}>
              {field.label}
